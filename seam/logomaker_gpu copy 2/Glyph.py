...
class Glyph:
    ...
    def draw(self):
        ...
        height = abs(self.ceiling - self.floor)
        x_center = self.p + 0.5
        y_center = (self.ceiling + self.floor) / 2
        ...
        transformation = Affine2D() \
            .scale(sx=self.width, sy=height) \
            .translate(tx=x_center, ty=y_center)
        
        # Apply transformation
        transformed_path = transformation.transform_path(path)
        
        # Create and add the patch
        self.patch = PathPatch(transformed_path,
    ...
    def _make_patch(self):
        ...
        # Get or create text path with timing
        t0 = time.time()
        cache_key = (self.c, self.font_name)
        if cache_key not in self._path_cache:
            print(f"Cache miss for {cache_key}")  # Debug cache misses
            tmp_path = TextPath((0, 0), self.c, size=1, prop=self.prop)
            self._path_cache[cache_key] = tmp_path.deepcopy()  # Store a clean copy
            self._bbox_cache[cache_key] = tmp_bbox = tmp_path.get_extents()
        else:
            tmp_path = self._path_cache[cache_key].deepcopy()  # Always work with a copy
        ...
        if self.font_name not in self._m_path_cache:
            ...
            self._bbox_cache[('M', self.font_name)] = msc_path.get_extents()
        msc_path = self._m_path_cache[self.font_name]
        msc_bbox = self._bbox_cache[('M', self.font_name)]
