...
def heatmap_pairwise(values,
    ...
    if not show_seplines: # Plot an outline around the triangular boundary
        ...
        ax.plot(bottom_x, bottom_y, **sepline_kwargs)

        # Fill in remaining segment
        last_x = [top_x[0], bottom_x[0]]
        last_y = [top_y[0], bottom_y[0]]
        ax.plot(last_x, last_y, **sepline_kwargs)
