...
class TF2DeepExplainer(Explainer):
    ...
    def _compute_gradients(self, joint_input, output_idx):
        print("\nComputing gradients...")
        print("Input shape to gradient computation:", joint_input[0].shape)
        
        # Reshape input to match model's expected shape
        reshaped_input = tf.reshape(joint_input[0], [-1, 249, 4])
        ...
        with tf.GradientTape() as tape:
            ...
            if self.multi_output:
                ...
                selected = outputs
            print("Selected output shape:", selected.shape)
            
        gradients = tape.gradient(selected, reshaped_input)
        print("Raw gradient shape:", gradients.shape)
        
        # Reshape gradients back to match joint_input shape
        gradients = tf.reshape(gradients, [-1, 249, 4])
        print("Final gradient shape:", gradients.shape)
        print("First few gradient values:", gradients[:5, 0, :])
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", batch_size=50):
        ...
        print("\n=== TF2 Batch Implementation Debug ===")
        ...
        for output_idx in range(self.num_outputs):
            ...
            for batch_start in range(0, num_samples, batch_size):
                ...
                print("\nBatch X shapes:", [bx.shape for bx in batch_X])
                
                # Tile the input for samples
                tiled_X = []
                for x in batch_X:
                    if len(x.shape) == 3:  # sequence data
                        # Tile to match n_backgrounds while keeping the sequence shape
                        x_squeezed = tf.squeeze(x, axis=0)  # Remove batch dim: [249, 4]
                        x_tiled = tf.tile(tf.expand_dims(x_squeezed, 0), [n_backgrounds, 1, 1])  # [n_backgrounds, 249, 4]
                        tiled_X.append(x_tiled)
                    else:  # regular data
                        x_squeezed = tf.squeeze(x, axis=0)
                        x_tiled = tf.tile(tf.expand_dims(x_squeezed, 0), [n_backgrounds, 1, 1])
                        tiled_X.append(x_tiled)
                print("Tiled X shapes:", [tx.shape for tx in tiled_X])
                
                # Reshape background data for samples
                bg_data_reshaped = []
                for bg in self.bg_data:
                    bg_data_reshaped.append(bg)  # Already in correct shape [n_backgrounds, 249, 4]
                print("Reshaped background shapes:", [bg.shape for bg in bg_data_reshaped])
                
                # Combine the samples (concatenate like sess)
                joint_x = [tf.concat([tx, bg], axis=0) for tx, bg in zip(tiled_X, bg_data_reshaped)]
                print("Joint input shapes:", [jx.shape for jx in joint_x])
                print("\nFirst few entries of joint_x:")
                print("Original data (first 3 entries):\n", joint_x[0][:3, 0, :])
                print("Background data (first 3 entries):\n", joint_x[0][n_backgrounds:n_backgrounds+3, 0, :])
                print("\nFull slice at first position:\n", joint_x[0][:, 0, :])
                
                try:
                    with tf.GradientTape(persistent=True) as tape:
                        grads = self._compute_gradients(joint_x, output_idx)
                        print("\nGradient shapes:", grads.shape)
                        print("First few gradient values:", grads[:5])
                ...
                print("\nMultiplier shapes:", [m.shape for m in multipliers])
                print("First few multiplier values:", multipliers[0][:5])
                ...
                print("\nFinal values shape:", values[0].shape)
                print("First few final values:", values[0][:5])
