...
class TF2DeepExplainer(Explainer):
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", batch_size=50):
        ...
        # Add debug prints
        print("Input X shape:", [x.shape for x in X])
        print("Background data shape:", [bg.shape for bg in self.bg_data])
        ...
        for output_idx in range(self.num_outputs):
            ...
            for batch_start in range(0, num_samples, batch_size):
                ...
                print("Batch X shape:", [x.shape for x in batch_X])
                
                # Create hypothetical inputs for each feature
                n_features = batch_X[0].shape[-1]
                hypothetical_inputs = []
                
                # For sequence data (shape [batch, seq_len, features])
                if len(batch_X[0].shape) == 3:
                    seq_len = batch_X[0].shape[1]
                    for pos in range(seq_len):
                        for feat in range(n_features):
                            # Create one-hot tensor: [batch_size, seq_len, features]
                            hypothetical = tf.zeros_like(batch_X[0])
                            indices = [[b, pos, feat] for b in range(batch_size_actual)]
                            hypothetical = tf.tensor_scatter_nd_update(
                                hypothetical,
                                indices,
                                tf.ones([batch_size_actual])
                            )
                            # Add background dimension: [batch_size, n_backgrounds, seq_len, features]
                            hypothetical = tf.expand_dims(hypothetical, 1)
                            hypothetical = tf.tile(hypothetical, [1, n_backgrounds, 1, 1])
                            hypothetical_inputs.append(hypothetical)
                else:
                    ...
                    for i in range(n_features):
                        ...
                        hypothetical = tf.tensor_scatter_nd_update(
                            hypothetical,
                            [[b, i] for b in range(batch_size_actual)],
                            tf.ones([batch_size_actual])
                        )
                        hypothetical = tf.expand_dims(hypothetical, 1)
                        hypothetical = tf.tile(hypothetical, [1, n_backgrounds, 1])
                        hypothetical_inputs.append(hypothetical)
                
                # Stack hypotheticals
                hypothetical_inputs = tf.stack(hypothetical_inputs)
                print("Hypothetical inputs shape:", hypothetical_inputs.shape)
                
                # Compute differences from background
                bg_expanded = tf.expand_dims(self.bg_data[0], 0)  # [1, n_backgrounds, n_features]
                bg_tiled = tf.tile(bg_expanded, [batch_size_actual, 1, 1])  # [batch_size, n_backgrounds, n_features]
                differences = hypothetical_inputs - tf.expand_dims(bg_tiled, 0)  # [n_features, batch_size, n_backgrounds, n_features]
                
                # Compute gradients for all hypothetical inputs at once
                with tf.GradientTape() as tape:
                    # Reshape for model: [n_features*batch_size*n_backgrounds, n_features]
                    inputs_reshaped = tf.reshape(hypothetical_inputs, [-1, hypothetical_inputs.shape[-1]])
                    tape.watch(inputs_reshaped)
                    outputs = self.model(inputs_reshaped)
                    selected = outputs[:, output_idx] if self.multi_output else outputs
                    
                # Get gradients and reshape back
                grads = tape.gradient(selected, inputs_reshaped)
                grads = tf.reshape(grads, [n_features, batch_size_actual, n_backgrounds, n_features])
                
                # Compute contributions
                contributions = differences * grads
                
                # Sum across feature dimension (axis=3) and mean across backgrounds (axis=2)
                values = tf.reduce_mean(tf.reduce_sum(contributions, axis=3), axis=2)
                # Transpose to get [batch_size, n_features]
